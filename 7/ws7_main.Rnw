...
\fi
    ...
    \part
      ...
      \begin{solution}
        ...
        \colon \text{``not all means the same''}\), where \(\mu_1, \dots, \mu_6\) are the
...
@
        ...
        analysis of variance is not very useful. As a follow-up, we run Tukey's honest
        significantly difference (HSD) test to determine which groups differ from each
        other:
<<cuckoo-tukey>>=
...
@
    ...
    \part
      ...
      \begin{solution}
        % NOTE: A "the" is missing from "not all medians [the] same" to reduce length and
        % avoid overfull \hbox. Maybe using \text here is not the best.
        We are interested in testing \(H_0 \colon M_1 = \dots = M_6\) against \(H_a \colon
        \text{``not all medians same''}\), where \(M_1, \dots, M_6\) are the population
        medians of the cuckoo egg lengths for each of the six host species. We perform
        Mood's median test to determine whether or not to reject \(H_0\):
<<cuckoo-median-test>>=
