...
<<senior-phone-t-test>>=
(call.t.test <- t.test(senior.phone$call_length, mu = 9.2))
@
        We see that our \(p\)\nobreakdash-value is around \num[round-mode = figures,
        round-precision = 2]{\Sexpr{call.t.test$p.value}}. Assuming a significance level
...
\fi
    ...
    \part
      ...
      \begin{solution}
        ...
        is suitable when the data is normally distributed or when the sampling
...
@
        ...
        To verify whether the sampling distributions of the sample means for the
        \num{3}\nobreakdash-bedroom and \num{4}\nobreakdash-bedroom groups are normal, we
        can draw bootstrap samples:
<<homes-bootstrap>>=
`3.homes` <- homes.tidy %>% filter(bdrms == 3)
`4.homes` <- homes.tidy %>% filter(bdrms == 4)

set.seed(25)  # for reproducibility with sample function
tibble(.rows = 1000) %>%
  rowwise() %>%
...
@
        We draw a histogram of the bootstrapped sample means for \num{3}\nobreakdash-bedroom
        homes (see \autoref{fig:3bdrms-bootstrap} for the outputted histogram):
<<homes-bootstrap-3-bedrooms-plot, eval = FALSE>>=
ggplot(homes.bs, aes(`3.mean`)) +
  geom_histogram(bins = 10) +
  labs(x = "Mean asking price ($)", y = "Frequency")
@
        Similarly for \num{4}\nobreakdash-bedroom homes (see \autoref{fig:4bdrms-bootstrap}
        for the outputted histogram):
<<homes-bootstrap-4-bedrooms-plot, eval = FALSE>>=
ggplot(homes.bs, aes(`4.mean`)) +
  geom_histogram(bins = 10) +
  labs(x = "Mean asking price ($)", y = "Frequency")
@
        We can see from our histograms that the sampling distributions appear fairly
        normal. Thus, a \(t\)\nobreakdash-test might actually be suitable as the samples
        appear large enough to overcome their skewness and outliers.
      \end{solution}

\ifprintanswers
  \begin{figure}[h]
    \centering
<<homes-bootstrap-3-bedrooms-plot, echo = FALSE, message = FALSE>>=
@
    \caption{Bootstrapped mean asking prices of \num{3}\nobreakdash-bedroom homes}
    \label{fig:3bdrms-bootstrap}
  \end{figure}

  \begin{figure}[h]
    \centering
<<homes-bootstrap-4-bedrooms-plot, echo = FALSE, message = FALSE>>=
@
    \caption{Bootstrapped mean asking prices of \num{4}\nobreakdash-bedroom homes}
    \label{fig:4bdrms-bootstrap}
  \end{figure}
\fi
...
@
        ...
        We see that our \(p\)\nobreakdash-value is around \Sexpr{signif(homes.t.test$p.value,
        3)}. Assuming a significance level (\(\alpha\)) of \num{0.05}, \(p \leq \alpha\);
        the evidence against \(H_0\) is weak, so we reject it. In other words, we can
        conclude that mean (log--)asking price for \num{4}\nobreakdash-bedroom homes is
