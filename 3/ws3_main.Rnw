...
\titledquestion{Student stats}
  ...
  \begin{parts}
    ...
    \part
      ...
      \begin{solution}
        We read the CSV file into a variable from the URL above and display part of its
        content:
...
@
    ...
    \part
      ...
      together).
      \begin{solution}
        We create a new data frame summarizing the mean and standard deviation of sleep
        times:
<<ucdavis-mean-sd>>=
...
@
    ...
    \part
      ...
      \begin{solution}
        We create a new data frame summarizing the number of students and their mean
        sleep time for each classroom seating group:
<<ucdavis-seats-summary>>=
ucdavis %>%
  group_by(Seat) %>%
...
@
    ...
    \part
      ...
      \begin{solution}
        ...
        reason or another (e.g., they are an orphan, opted to not input their parents'
        heights):
<<ucdavis-show-na>>=
ucdavis %>%
...
@
        ...
        be reasonable if values being missing actually informs something about heights,
        e.g., being ashamed of their parents being too short and preferring not to share
        their heights.) Otherwise, our mean heights may be \texttt{NA} as well. We filter
...
ucdavis %>%
  ...
  summarize(across(c(momheight, dadheight),
                   ...
                   .names = "{.col}.mean"))
@
        Note that there are a few different ways of dropping missing values. What follows is a method using \texttt{drop\_na}:
