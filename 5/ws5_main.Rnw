      ...
      \begin{solution}
        ...
        \begin{itemize}
          ...
          \item Count how many times we correctly rejected \(H_0\) given a significance
            level of~\(\alpha = 0.05\)
        \end{itemize}
<<child-psychology-simulation-power>>=
(tibble(.rows = 1000) %>%
  rowwise() %>%
  mutate(sample = list(rnorm(20, 12, 2.3)),
         ttest = list(t.test(sample, alternative = "greater", mu = 11)),
         pvalue = ttest$p.value) %>%
  ungroup() %>%  # for later column-wise operations on psych.sim
  count(pvalue <= 0.05) -> psych.sim)
@
        Assuming the mean is actually \num{12}~minutes, we can see that we correctly
        rejected the null hypothesis \num{\Sexpr{psych.sim %>% filter(`pvalue <= 0.05`) %>%
        pull(n)}}~times out of~\num{\Sexpr{psych.sim %>% summarize(sum(n))}}. Thus, the
        estimated power of our test is~\num{\Sexpr{psych.sim %>% filter(`pvalue <= 0.05`) %>%
        pull(n) / psych.sim %>% summarize(sum(n))}}.
      \end{solution}
...
@
        ...
        estimated power of~\num{\Sexpr{psych.sim %>% filter(`pvalue <= 0.05`) %>%
        pull(n) / psych.sim %>% summarize(sum(n))}} in Part~\ref{part@2@1}.
      \end{solution}
...
(tibble(.rows = 1000) %>%
  ...
  mutate(sample1 = list(rnorm(30, 20, 9)),
         ...
         pvalue = ttest$p.value) %>%
  ungroup() %>%  # for later column-wise operations on twosam.sim
  count(pvalue <= 0.05) -> twosam.sim)
@
        We can see that we correctly rejected the null hypothesis \num{\Sexpr{twosam.sim %>%
        filter(`pvalue <= 0.05`) %>% pull(n)}}~times out of~\num{\Sexpr{twosam.sim %>%
        summarize(sum(n))}}. Thus, the estimated power of our two-sample \(t\)\nobreakdash-test
        is~\num{\Sexpr{twosam.sim %>% filter(`pvalue <= 0.05`) %>% pull(n) /
        twosam.sim %>% summarize(sum(n))}}. This is fairly close to the \num{\Sexpr{
        twosam.pow$power}}~power calculated in Part~\ref{part@3@1}.
      \end{solution}
