...
\fi
    ...
    \part
      ...
      \begin{solution}
        ...
        course are also hosted on GitHub, which is another way of sharing them across
        devices.
      \end{solution}
...
@
        ...
        with parentheses, to avoid an extra line of code (this code chunk is just for
        example, so it does not run on purpose):
<<jumping-paren, eval = FALSE>>=
(rats <- read_delim(my_url, delim = " "))
@
      \end{solution}
